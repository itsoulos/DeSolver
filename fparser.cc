...
bool FunctionParser::Compile(const char* Function)
{
    ...
    vector<unsigned> byteCode; byteCode.reserve(10240);
		...
		break;
          ...
          case  cSqrt: if(Stack[SP] < 0) { evalErrorType=2; return 0; }
			a=Stack[SP];
		       d1=derivStack.pop();
		       d2=d1/(2.0*sqrt(a));
//		       if(sqrt(a)==0.0) {printf("abort\n");evalErrorType=2; return 0;}
		       derivStack.push(d2);
...
// Operators:
          ...
          case   cMod: if(Stack[SP] == 0) { evalErrorType=1; return 0; }
                       ...
                       --SP; break;
          case   cPow: a=Stack[SP];
		       b=Stack[SP-1];
		       Stack[SP-1] = pow(Stack[SP-1], Stack[SP]);
                       --SP; 
		       d1=derivStack.pop();
		       d2=derivStack.pop();
		       derivStack.push(pow(b,a)*(d1*log(b)+(a*d2)/b));
		       break;
...
// Variables:
          default:
	      if((ByteCode[IP]-VarBegin)==pos) derivStack.push(1.0);
	      else
		      derivStack.push(0.0);
...
// Functions:
          ...
          case cAsinh: Stack[SP] = asinh(Stack[SP]); break;
#endif
          case  cAtan: a=Stack[SP];
		       Stack[SP] = atan(Stack[SP]); 
		       d1=derivStack.pop();
		       d2=d1/(1+a*a);
		       dd1=derivStack2.pop();
		       derivStack.push(d2);
		       dd2=(dd1*(1.0+a*a)-2.0*a*d1*d1)/((1+a*a)*(1+a*a));
          ...
          case   cCos: 
		       ...
		       d2=-d1 * sin(a);
		       derivStack.push(d2);
		       dd2=-dd1 * sin(a)-d1 * d1 * cos(a);
			if(isnan(d2) || isinf(d2) || isnan(dd2) || isinf(dd2)) 
			{
				evalErrorType=3;
				return 0;
			}
		...
		break;
          ...
          case  cSqrt: if(Stack[SP] < 0) { evalErrorType=2; return 0; }
			a=Stack[SP];
		       d1=derivStack.pop();
		       dd1=derivStack2.pop();
		       if(sqrt(a)==0.0) {evalErrorType=2; return 0;}
		       d2=d1/(2.0*sqrt(a));
		       derivStack.push(d2);
		       dd2=(2.0*dd1*sqrt(a)-d1*d1/sqrt(a))/(4.0*sqrt(a)*sqrt(a));
		       derivStack2.push(dd2);
                       Stack[SP] = sqrt(Stack[SP]); 
		       break;
          case   cTan: Stack[SP] = tan(Stack[SP]); break;
...
// Misc:
          case cImmed: 
		       derivStack.push(0.0);
		       derivStack2.push(0.0);
		       evalStack.push(Immed[DP++]); 
		       break;
          case  cJump: DP = ByteCode[IP+2];
                       IP = ByteCode[IP+1];
                       break;
...
// Operators:
          ...
          case   cDiv: 
		       ...
		       dd1=derivStack2.pop();
		       dd2=derivStack2.pop();
		       dd2=(((dd2*a-dd1*b)*a-2.0*d1*(d2*a-d1*b))/(a*a*a));
		       d2=(d2*a-d1*b)/(a*a); 
          ...
          case   cPow: a=Stack[SP];
		       ...
		       dd2=derivStack2.pop();
		       derivStack.push(pow(b,a)*(d1*log(b)+(a*d2)/b));
		       derivStack2.push(pow(b,a)*pow(d1*log(b)+(a*d2)/b,2.0)
		        +pow(b,a)*(dd2*log(b)+d1*d2/b+(b*d2*d1+a*b*dd2-a*d2*d2)/(b*b)));
		       break;
...
// Variables:
	      ...
	      if((ByteCode[IP]-VarBegin)==pos) derivStack.push(1.0);
		      ...
		      derivStack.push(0.0);
	      derivStack2.push(0.0);
              evalStack.push( Vars[ByteCode[IP]-VarBegin]);
        }
    }

    evalErrorType=0;
    lastValue=evalStack.top();
    lastDeriv=derivStack.top();
    return derivStack2.top();
}
